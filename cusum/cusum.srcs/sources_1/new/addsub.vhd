                ...
                when S_READ =>
                    if external_ready = '1' and inputs_valid = '1' then
                        if s_axis_operation_tdata = "00000000" then
                            ...
                            report "ADDITION PERFORMED BETWEEN A = " & integer'image(to_integer(unsigned(s_axis_a_tdata))) & " AND B = " & integer'image(to_integer(unsigned(s_axis_b_tdata)));
                        else
                            result <= s_axis_a_tdata - s_axis_b_tdata;
                            report "SUBSTRACTION PERFORMED BETWEEN A = " & integer'image(to_integer(unsigned(s_axis_a_tdata))) & " AND B = " & integer'image(to_integer(unsigned(s_axis_b_tdata)));
                        end if;
